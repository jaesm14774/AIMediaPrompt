---
name: health
description: Audit Codex config health across all layers.
version: "1.5.0"
disable-model-invocation: true
---

# Codex Configuration Health Audit

Systematically audit the current project's Codex setup using the six-layer framework:
...
**Output language:** 使用繁體中文輸出所有診斷結果。Detect from the conversation language or the AGENTS.md `## Communication` rule. This project defaults to Traditional Chinese (繁體中文).
...
|------|--------|-----------------|
| **Simple** | <500 source files, 1 contributor, no CI | AGENTS.md only; 0–1 skills; no rules/; hooks optional |
| **Standard** | 500–5K files, small team or CI present | AGENTS.md + 1–2 rules files; 2–4 skills; basic hooks |
...
**Note:** This script is adapted for Windows/Git Bash. It uses `python` (not `python3`) and handles `jq` absence gracefully. Skill files are searched as both `SKILL.md` and `skill.md`.

```bash
P=$(pwd)
SETTINGS="$P/.Codex/settings.local.json"
PY=$(command -v python3 2>/dev/null || command -v python 2>/dev/null || echo "python")

echo "=== TIER METRICS ==="
echo "source_files: $(find "$P" -type f \( -name "*.rs" -o -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.py" -o -name "*.go" -o -name "*.lua" -o -name "*.swift" -o -name "*.rb" -o -name "*.java" -o -name "*.kt" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" \) -not -path "*/.git/*" -not -path "*/node_modules/*" 2>/dev/null | wc -l)"
...
echo "skills:        $(find "$P/.Codex/skills" \( -name "SKILL.md" -o -name "skill.md" \) 2>/dev/null | grep -vi '/health/' | wc -l)"
echo "claude_md_lines: $(wc -l < "$P/AGENTS.md" 2>/dev/null || echo 0)"

echo "=== AGENTS.md (global) ===" ; cat ~/.Codex/AGENTS.md 2>/dev/null || echo "(none)"
echo "=== AGENTS.md (local) ===" ; cat "$P/AGENTS.md" 2>/dev/null || echo "(none)"
echo "=== .claudeignore ===" ; cat "$P/.claudeignore" 2>/dev/null || echo "(none)"
echo "=== settings.local.json ===" ; cat "$SETTINGS" 2>/dev/null || echo "(none)"
echo "=== rules/ ===" ; find "$P/.Codex/rules" -name "*.md" 2>/dev/null | while IFS= read -r f; do echo "--- $f ---"; cat "$f"; done
if [ -z "$(find "$P/.Codex/rules" -name "*.md" 2>/dev/null)" ]; then echo "(no rules/ directory or no .md files)"; fi
echo "=== skill descriptions ===" ; grep -r "^description:" "$P/.Codex/skills" ~/.Codex/skills 2>/dev/null
echo "=== STARTUP CONTEXT ESTIMATE ==="
echo "global_claude_words: $(wc -w < ~/.Codex/AGENTS.md 2>/dev/null | tr -d ' ' || echo 0)"
echo "local_claude_words: $(wc -w < "$P/AGENTS.md" 2>/dev/null | tr -d ' ' || echo 0)"
echo "rules_words: $(find "$P/.Codex/rules" -name "*.md" 2>/dev/null | while IFS= read -r f; do cat "$f"; done | wc -w | tr -d ' ')"
echo "skill_desc_words: $(grep -r "^description:" "$P/.Codex/skills" ~/.Codex/skills 2>/dev/null | wc -w | tr -d ' ')"
echo "=== hooks ===" ; $PY -c "import json,sys; d=json.load(open('$SETTINGS')); print(json.dumps(d.get('hooks',{}), indent=2))" 2>/dev/null || echo "(unavailable: settings.local.json missing or malformed)"
echo "=== MCP ===" ; $PY -c "
import json
try:
...
echo "=== allowedTools count ===" ; $PY -c "import json; d=json.load(open('$SETTINGS')); print(len(d.get('permissions',{}).get('allow',[])))" 2>/dev/null || echo "(unavailable)"
echo "=== allowedTools list ===" ; $PY -c "import json; d=json.load(open('$SETTINGS')); [print(x) for x in d.get('permissions',{}).get('allow',[])]" 2>/dev/null || echo "(unavailable)"
echo "=== NESTED AGENTS.md ===" ; find "$P" -name "AGENTS.md" -not -path "$P/AGENTS.md" -not -path "*/.git/*" -not -path "*/node_modules/*" 2>/dev/null || echo "(none)"
echo "=== GITIGNORE ===" ; (grep -qE "settings\.local" "$P/.gitignore" "$P/.Codex/.gitignore" 2>/dev/null && echo "settings.local.json: gitignored") || echo "settings.local.json: NOT gitignored -- risk of committing tokens/credentials"
echo "=== HANDOFF.md ===" ; cat "$P/HANDOFF.md" 2>/dev/null || echo "(none)"
echo "=== MEMORY.md ===" ; cat "$HOME/.Codex/projects/-$(pwd | sed 's|[/_]|-|g; s|^-||')/memory/MEMORY.md" 2>/dev/null | head -50 || echo "(none)"

echo "=== CONVERSATION FILES ==="
PROJECT_PATH=$(pwd | sed 's|[/_]|-|g; s|^-||')
CONVO_DIR=~/.Codex/projects/-${PROJECT_PATH}
ls -lhS "$CONVO_DIR"/*.jsonl 2>/dev/null | head -10 || echo "(no conversation files)"
...
if [ -n "$FILES" ]; then
  if command -v jq &>/dev/null; then
    for F in $FILES; do
      ...
      ' 2>/dev/null | grep -v "^ASSISTANT: $" | head -300 || echo "(parse error)"
    done
  else
    echo "(jq not installed -- falling back to python extraction)"
    for F in $FILES; do
      echo "--- file: $F ---"
      $PY -c "
import json, sys
    ...
    try:
        ...
        if t == 'user':
            ...
            if isinstance(content, list):
                ...
                text = str(content)
            if text.strip(): print(f'USER: {text[:500]}')
        elif t == 'assistant':
            content = obj.get('message',{}).get('content',[])
            if isinstance(content, list):
                ...
                text = str(content)
            if text.strip(): print(f'ASSISTANT: {text[:500]}')
    except: pass
" 2>/dev/null | head -300 || echo "(extraction failed)"
    done
  fi
else
...
SELF_SKILL=$(grep -rl '^name: health$' "$P/.Codex/skills" "$HOME/.Codex/skills" 2>/dev/null | grep -iE 'SKILL\.md|skill\.md' | head -1)
[ -z "$SELF_SKILL" ] && SELF_SKILL="health/skill.md"

# Search both SKILL.md and skill.md (case-insensitive)
SKILL_FIND_PATTERN='\( -name "SKILL.md" -o -name "skill.md" \)'

echo "=== SKILL INVENTORY ==="
for DIR in "$P/.Codex/skills" "$HOME/.Codex/skills"; do
  [ -d "$DIR" ] || continue
  find -L "$DIR" \( -name "SKILL.md" -o -name "skill.md" \) 2>/dev/null | grep -v "$SELF_SKILL" | while IFS= read -r f; do
    WORDS=$(wc -w < "$f" | tr -d ' ')
    IS_LINK="no"; LINK_TARGET=""
    SKILL_DIR=$(dirname "$f")
    if [ -L "$SKILL_DIR" ]; then
      IS_LINK="yes"; LINK_TARGET=$(readlink -f "$SKILL_DIR" 2>/dev/null || readlink "$SKILL_DIR" 2>/dev/null || echo "unknown")
    fi
...
for DIR in "$P/.Codex/skills" "$HOME/.Codex/skills"; do
  ...
  find "$DIR" -maxdepth 1 -type l 2>/dev/null | while IFS= read -r link; do
    TARGET=$(readlink -f "$link" 2>/dev/null || readlink "$link" 2>/dev/null || echo "unknown")
...
[PASTE Step 1 output sections: AGENTS.md (global), AGENTS.md (local), .claudeignore, NESTED AGENTS.md, rules/, skill descriptions, STARTUP CONTEXT ESTIMATE, MCP, HANDOFF.md, MEMORY.md, SKILL INVENTORY, SKILL SECURITY SCAN, SKILL FRONTMATTER, SKILL SYMLINK PROVENANCE, SKILL FULL CONTENT]
...
Tier-adjusted AGENTS.md checks:
- ALL tiers: Is AGENTS.md short and executable? No prose, no background, no soft guidance.
- ALL tiers: Does it have build/test commands?
- ALL tiers: Flag any nested AGENTS.md files found in subdirectories -- stacked context causes unpredictable behavior.
- ALL tiers: Compare global and local AGENTS.md for duplicate rules (same constraint in both = wasted context) and conflicting rules (opposite directives = unpredictable behavior). Duplicates are 🟢, conflicts are 🔴.
...
Tier-adjusted .claudeignore checks:
- ALL tiers: Does .claudeignore exist? Is it effective at excluding large/irrelevant files?
- Flag if node_modules, dist, build outputs, or binary files are NOT excluded.
- Flag if it's overly aggressive (excluding source code that Codex might need).

Tier-adjusted rules/ checks:
- SIMPLE: rules/ is NOT required — do not flag its absence.
- STANDARD+: Language-specific rules like Rust or Lua should be in rules/ not AGENTS.md.
- COMPLEX: Path-specific rules should be isolated; no rules in root AGENTS.md.
...
- Check if project has `.Codex/projects/.../memory/MEMORY.md`
- Verify AGENTS.md references MEMORY.md for architecture decisions
...
- Verify AGENTS.md includes "AGENTS.md 使用指南" section
...
- Flag if AGENTS.md alone > 5K tokens (~3800 words): contract is oversized

Tier-adjusted HANDOFF.md check STANDARD+:
- Check if HANDOFF.md exists or if AGENTS.md mentions handoff practice
...
- Flag done-conditions in AGENTS.md with no matching command in the project.
...
[CONTEXT LAYER: AGENTS.md issues | .claudeignore issues | rules/ issues | skill description issues | MCP cost | verifiers gaps]
...
[PASTE Step 1 output sections: settings.local.json, GITIGNORE, AGENTS.md (global), AGENTS.md (local), hooks, allowedTools count, allowedTools list, skill descriptions, CONVERSATION EXTRACT]
...
- ALL tiers: Flag hook commands without error surfacing -- commands that can fail silently (no || echo 'FAILED' or explicit exit handling) give Codex no signal that the check failed
...
- Check AGENTS.md or hooks for dynamic timestamps/dates injected into system-level context -- breaks prompt cache on every request
...
- For each critical rule in AGENTS.md NEVER/ALWAYS items, check if:
  1. AGENTS.md declares the rule: intent layer
...
- Flag rules that only exist in one layer — single-layer rules are fragile:
  - AGENTS.md-only rules: Codex may ignore them under context pressure
...
- SIMPLE: No formal verification section required. Only flag if Codex declared done without running any check.
- STANDARD+: AGENTS.md should have a Verification section with per-task done-conditions.
...
1. Rules violated: Find cases where AGENTS.md says NEVER/ALWAYS but Codex did the opposite. Quote both the rule and the violation. Only flag if directly observed, not inferred.
2. Repeated corrections: Find cases where the user corrected Codex's behavior more than once on the same issue. Requires evidence in at least 2 conversations to flag as repeated.
3. Missing local patterns: Find project-specific behaviors the user reinforced in conversation but that aren't in local AGENTS.md.
4. Missing global patterns: Find behaviors that would apply to any project that aren't in ~/.Codex/AGENTS.md.
...
6. Anti-patterns: Only flag what is directly observable in the sample:
   - Codex declaring done without running verification
...
[BEHAVIOR: rules violated | repeated corrections | add to local AGENTS.md | add to global AGENTS.md | skill frequency | anti-patterns -- each finding tagged with confidence level]
```
...
> "需要我草擬修改方案嗎？可以逐層處理：global AGENTS.md / local AGENTS.md / hooks / skills / .claudeignore"
