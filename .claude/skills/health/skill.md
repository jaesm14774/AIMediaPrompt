---
name: health
description: Audit Claude Code config health across all layers.
version: "1.5.0"
...
**Output language:** 使用繁體中文輸出所有診斷結果。Detect from the conversation language or the CLAUDE.md `## Communication` rule. This project defaults to Traditional Chinese (繁體中文).
...
echo "skills:        $(find "$P/.claude/skills" \( -name "SKILL.md" -o -name "skill.md" \) 2>/dev/null | grep -vi '/health/' | wc -l)"
...
if [ -z "$(find "$P/.claude/rules" -name "*.md" 2>/dev/null)" ]; then echo "(no rules/ directory or no .md files)"; fi
...
SELF_SKILL=$(grep -rl '^name: health$' "$P/.claude/skills" "$HOME/.claude/skills" 2>/dev/null | grep -iE 'SKILL\.md|skill\.md' | head -1)
...
[PASTE Step 1 output sections: CLAUDE.md (global), CLAUDE.md (local), .claudeignore, NESTED CLAUDE.md, rules/, skill descriptions, STARTUP CONTEXT ESTIMATE, MCP, HANDOFF.md, MEMORY.md, SKILL INVENTORY, SKILL SECURITY SCAN, SKILL FRONTMATTER, SKILL SYMLINK PROVENANCE, SKILL FULL CONTENT]
...
- Flag if it's overly aggressive (excluding source code that Claude might need).
...
[CONTEXT LAYER: CLAUDE.md issues | .claudeignore issues | rules/ issues | skill description issues | MCP cost | verifiers gaps]
...
[PASTE Step 1 output sections: settings.local.json, GITIGNORE, CLAUDE.md (global), CLAUDE.md (local), hooks, allowedTools count, allowedTools list, skill descriptions, CONVERSATION EXTRACT]
...
> "需要我草擬修改方案嗎？可以逐層處理：global CLAUDE.md / local CLAUDE.md / hooks / skills / .claudeignore"
